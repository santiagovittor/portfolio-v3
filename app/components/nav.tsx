...
export function Nav() {
  ...
  useEffect(() => {
    ...
    const check = () => {
      raf = 0;
      setOnPaper(window.scrollY > window.innerHeight * 0.9);
    };
    const onScroll = () => {
    ...
    <header className="fixed inset-x-0 top-0 z-40 flex items-center justify-between p-4 transition-colors duration-300 md:px-8 md:py-6">
      ...
      >
        <ul className={`flex items-center text-sm font-medium transition-colors duration-300 ${text}`}>
          <li>
            <a href="#work" className={`rounded-full px-4 py-2 transition-colors ${hover}`}>
              Work
            </a>
          </li>
          <li>
            <a href="#about" className={`rounded-full px-4 py-2 transition-colors ${hover}`}>
              About
            </a>
          </li>
          <li>
            <a href="#contact" className={`rounded-full px-4 py-2 transition-colors ${hover}`}>
              Contact
            </a>
          </li>
        </ul>
